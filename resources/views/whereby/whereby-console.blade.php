            ...
            <div>
                ...
                <div class="md:grid md:grid-cols-3 md:gap-6">
                    <div class="md:col-span-1 flex justify-between">
                        <div class="px-4 sm:px-0">
                            <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">Assign Whereby Credentials</h3>

                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                Assigning Whereby credentials entails granting a user an bearer token, enabling secure access to Whereby services.
                            </p>
                    ...
                    <div class="mt-5 md:mt-0 md:col-span-2">
                        <form action="/whereby-console" method="POST">
                        @csrf
                            <div class="px-4 py-5 bg-white dark:bg-gray-800 sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                                <div class="grid grid-cols-6 gap-6">
                                    ...
                                    <div class="col-span-6 sm:col-span-4">
                                        ...
                                        <select id="token_id" name="token_id" class="border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm mt-1 w-full" autofocus>
                                            ...
                                            @foreach($accessTokens as $accessToken)
                                                <option value="{{ $accessToken->id }}" style="@if(\App\Models\WBConsole::where('token_id', $accessToken->id)->exists())display:none;@endif">{{ $accessToken->name }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-span-6 sm:col-span-4">
                                        <x-label for="bearer" value="{{ __('Bearer Token') }}" />
                                        <x-input id="bearer" name="bearer" type="text" class="mt-1 block w-full" autofocus />
                ...
                <div class="mt-10 sm:mt-0">
                    <div class="md:grid md:grid-cols-3 md:gap-6">
                        <div class="md:col-span-1 flex justify-between">
                            <div class="px-4 sm:px-0">
                                <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ __('Manage Whereby Credentials') }}</h3>

                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    {{ __('Controlling bearer token by viewing or removing them as needed for users, ensuring secure and up-to-date access to Whereby services.') }}
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="mt-5 md:mt-0 md:col-span-2">
                            <div class="px-4 py-5 sm:p-6 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                                ...
                                @foreach($WBConsoles as $WBConsole)
                                    <div class="flex items-center justify-between">
                                        <div class="break-all dark:text-white">
                                        @php
                                            $accessToken = \DB::table('personal_access_tokens')
                                                ->where('id', $WBConsole->token_id)
                                                ->value('name');
                                                echo $accessToken;
                                        @endphp
                                        </div>
                                        ...
                                        <div class="flex items-center ml-2">
                                            <button class="cursor-pointer ml-6 text-sm text-gray-400 underline">{{ $WBConsole->created_at }}</button>
                                            <a href="/whereby-console/{{ $WBConsole->id }}" class="cursor-pointer ml-6 text-sm text-red-500">Delete</a>
                                        </div>
                                    </div>
                                @endforeach
